...
beforeEach(async () => {
  ...
  userData = join(appData, 'AI Usage Tracker')
})
...
describe('migrateLegacySettings', () => {
  it('brings the settings over to the new folder', async () => {
    ...
    expect(await migrateLegacySettings(appData, userData)).toBe(true)
  ...
  it('leaves stored tokens behind, since they could not be decrypted anyway', async () => {
    ...
    await migrateLegacySettings(appData, userData)
  ...
  it('never overwrites settings the new folder already has', async () => {
    ...
    expect(await migrateLegacySettings(appData, userData)).toBe(false)
